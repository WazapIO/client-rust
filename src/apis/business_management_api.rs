...
pub async fn send_payment_request(configuration: &configuration::Configuration, instance_key: &str, data: crate::models::PaymentRequestPayload) -> Result<crate::models::ApiResponse, Error<SendPaymentRequestError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/instances/{instance_key}/business/payment-request", local_var_configuration.base_path, instance_key=crate::apis::urlencode(instance_key));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<SendPaymentRequestError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
